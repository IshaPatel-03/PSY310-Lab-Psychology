...
expName = 'OrientationDiscrimination'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ishapatel/Desktop/PSY310_Lab/OrientationDiscrimination.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text="Instructions- You will be presented with a stimulus, and if it is tilted, then press key 'down', and if it is vertical (straight), press key 'up'.  When you are ready press SPACE to begin. ",
        font='Arial',
        pos=[0,0], draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    ins_resp = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # if Thank is starting this frame...
        ...
        # if Thank is stopping this frame...
        if Thank.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Thank.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
                Thank.tStop = t  # not accounting for scr refresh
                Thank.tStopRefresh = tThisFlipGlobal  # on global time
                Thank.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Thank.stopped')
                # update status
                Thank.status = FINISHED
