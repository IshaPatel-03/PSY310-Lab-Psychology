...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ishapatel/Desktop/PSY310_Lab/AdaptiveStaircase.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    ins = visual.TextStim(win=win, name='ins',
        text="Instructions- You will be presented with stimuli, if it is tilted towards left, press key 'left'; and if it is tilted towards right, press key 'right'. Press SPACE once you are ready.",
        font='Arial',
        pos=[0,0], draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    ins_res = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    Instruction = data.Routine(
        name='Instruction',
        components=[ins, ins_res],
    )
    Instruction.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for ins_res
    ins_res.keys = []
    ins_res.rt = []
    _ins_res_allKeys = []
    ...
    while continueRoutine:
        ...
        if ins.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            thisExp.timestampOnFlip(win, 'ins.started')
            # update status
            ins.status = STARTED
            ins.setAutoDraw(True)
        
        # if ins is active this frame...
        if ins.status == STARTED:
            # update params
            pass
        ...
        # if ins_res is starting this frame...
        if ins_res.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ins_res.frameNStart = frameN  # exact frame index
            ins_res.tStart = t  # local t and not account for scr refresh
            ins_res.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(ins_res, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'ins_res.started')
            # update status
            ins_res.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(ins_res.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(ins_res.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if ins_res.status == STARTED and not waitOnFlip:
            theseKeys = ins_res.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _ins_res_allKeys.extend(theseKeys)
            if len(_ins_res_allKeys):
                ins_res.keys = _ins_res_allKeys[-1].name  # just the last key pressed
                ins_res.rt = _ins_res_allKeys[-1].rt
                ins_res.duration = _ins_res_allKeys[-1].duration
    ...
    # check responses
    if ins_res.keys in ['', [], None]:  # No response was made
        ins_res.keys = None
    thisExp.addData('ins_res.keys',ins_res.keys)
    if ins_res.keys != None:  # we had a response
        thisExp.addData('ins_res.rt', ins_res.rt)
        thisExp.addData('ins_res.duration', ins_res.duration)
    thisExp.nextEntry()
    ...
    level = thisStair = 10  # initialise some vals
